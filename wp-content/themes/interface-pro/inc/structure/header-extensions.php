...
<?php
   	...
   	$elements = array();
		$elements = array( 	$options[ 'social_facebook' ], 
									...
									$options[ 'social_codepen' ],
									$options[ 'social_polldaddy' ],
									$options[ 'social_path' ],
									$options[ 'social_skype' ],
									$options[ 'social_digg' ],
									$options[ 'social_reddit' ],
									$options[ 'social_stumbleupon' ],
									$options[ 'social_pocket' ],
									$options[ 'social_dropbox' ]
							 	);	
...
<?php	
		...
		// Initializing the show breadcrumb variable to be true.
   	$interface_show_breadcrumb = 'true';
			if( 1 != $options[ 'disable_slider' ] ) {
   		if( 'revolution-slider' == $options[ 'slider_type' ] ) {
   			if( !empty( $options[ 'header_slider' ] ) && function_exists( 'putRevSlider' ) ) {
   				if( 1 == $options[ 'revslider_homepage' ] && ( is_home() || is_front_page() ) ) {
						...
						$interface_show_breadcrumb = 'false';
					}
					$interface_rev_page_array = explode( ',', $options[ 'pages_id_revslider' ] );
					if ( !empty( $options[ 'pages_id_revslider' ] ) && is_page( $interface_rev_page_array ) ) {
...
if ( ( 'slider-on-everypage' == $options[ 'slider_status' ] ) || ( ( is_home() || is_front_page() ) &&  'slider-on-homepage' == 
$options[ 'slider_status' ] ) ) {
   			if( function_exists( 'interface_pass_slider_effect_cycle_parameters' ) ) 
	   			...
	   			if( 'image-slider' == $options[ 'slider_type' ] && function_exists( 'interface_featured_post_image_slider' ) ) {
	   				interface_featured_post_image_slider();
	   				$interface_show_breadcrumb = 'false';
	   			}		   				
	   			if( 'upload-image-slider' == $options[ 'slider_type' ] && function_exists( 'interface_featured_image_slider' ) ) {
	   				interface_featured_image_slider();
	   				$interface_show_breadcrumb = 'false';
	   			}
   			}
   		}
...
if ( ! function_exists( 'interface_featured_post_image_slider' ) ) :
/**
 * display featured post slider
 */
function interface_featured_post_image_slider() {	
	global $post;
	global $array_of_default_settings;
 	$options = wp_parse_args( get_option( 'interface_theme_options', array() ), interface_get_option_defaults());
...
if ( ! function_exists( 'interface_featured_image_slider' ) ) :
/**
 * display featured post slider
 */
function interface_featured_image_slider() {	
	global $array_of_default_settings;
 	$options = wp_parse_args( get_option( 'interface_theme_options', array() ), interface_get_option_defaults());
	$interface_featured_image_slider = '';
		if( $interface_featured_image_slider != 'interface_featured_image_slider' ) {
				//print_r($options); die();
		...
		<section class="featured-slider"><div class="slider-cycle">';
			...
			if( !empty( $options[ 'featured_image_slider_link' ][ $i ] ) ) { $redirect_link = $options[ 'featured_image_slider_link' ][ $i ]; } else { $redirect_link = '#'; }
			if( !empty( $options[ 'featured_image_slider_title' ][ $i ] ) ) { $title = $options[ 'featured_image_slider_title' ][ $i ]; } else { $title = ''; }
			if ( 1 == $i ) { $classes = "slides displayblock"; } else { $classes = "slides displaynone"; }
			$interface_featured_image_slider .= '<div class="'.$classes.'">';

			if( !empty( $options[ 'featured_image_slider_image' ][ $i ] ) ) {
				$interface_featured_image_slider .= '<figure><a href="' . esc_url( $redirect_link ) .'" title="' . esc_attr( $title ) . '" >';
				$interface_featured_image_slider .= '<img alt="' . esc_attr( $title ) . '" src="' . $options[ 'featured_image_slider_image' ][ $i ] . '" >';
				$interface_featured_image_slider .= '</a></figure>';
			}

			if( !empty( $options[ 'featured_image_slider_title' ][ $i ] ) || !empty( $options[ 'featured_image_slider_description' ][ $i ] ) ) {
				$interface_featured_image_slider .= '<div class="container">
				<article class="featured-text">';
				if( $title != '' ) {
					$interface_featured_image_slider .= '<h2 class="featured-title"><a href="' . esc_url( $redirect_link ) .'" title="' . esc_attr( $title ) . '" >' . $title .'</a></h2><!-- .featured-title -->';
				}
				if( $options[ 'featured_image_slider_description' ][ $i ] !='' ) {				
					$interface_featured_image_slider .= '<div class="featured-content">'.$options[ 'featured_image_slider_description' ][ $i ].'</div><!-- .featured-content -->';
				}
				$interface_featured_image_slider .= '</article><!-- .featured-text -->';
				$interface_featured_image_slider .= '</div>';
			}

			$interface_featured_image_slider .= '</div><!-- .slides -->';
		}
		$interface_featured_image_slider .= '</div>				
		<nav id="controllers" class="clearfix">
		</nav><!-- #controllers --></section><!-- .featured-slider -->';
	...
	echo $interface_featured_image_slider;	
}
endif;
/****************************************************************************************/
