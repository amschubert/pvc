 ...
 class interface_Widget_Testimonial extends WP_Widget {
 	...
 	function form( $instance ) {
 		...
 		<?php for ( $i=1; $i<=$number; $i++ ) {
 			...
 			?>
 			<p>
 				<input class="upload1" type="text"  id="<?php echo $this->get_field_id( $image ); ?>" name="<?php echo $this->get_field_name($image); ?>" value="<?php if(isset ( $instance[$image] ) ) 
 				echo esc_url( $instance[$image] ); ?>" />
 				<input class="button custom_media_button" name="image-add" id="custom_media_button_services" type="button" value="<?php esc_attr_e( 'Add Image', 'interface' ); ?>" onclick="mediaupload.uploader( '<?php echo $this->get_field_id( $image ); ?>' ); return false;" />
 			</p>
